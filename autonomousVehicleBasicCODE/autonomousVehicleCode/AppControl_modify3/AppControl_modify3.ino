...
int angle_limit = 45; // 서보 모터 회전 제한 각 (단위: 도)
...
//int center_stop = 85; // 전방 멈춤 거리 (단위: mm)
int center_stop = 85; // 전방 멈춤 거리 (단위: mm)
int diagonal_detect = 75; // 대각 감지 거리 (단위: mm)
int diagonal_start = 100; // 대각 출발 거리 (단위: mm)
...
//            }
        ...
        else
        {
                ...
                if(f_left < f_right) // 좌측방이 감지되었다면
                    {
                        ...
                        {
                          f_right = GetDistance(FC_TRIG, FC_ECHO);
                          f_left = GetDistance(FC_TRIG, FC_ECHO);
                        }
                        else
                        {
                          f_right = 0;
            ...
            if(left <= side_start && right <= side_start) // 좌우 모두 감지되면
            {
                ...
                compute_steering = diff * steering_gain * 0.3;
            }
